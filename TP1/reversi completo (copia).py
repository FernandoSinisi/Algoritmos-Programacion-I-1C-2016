...
def hay_jugada_posible(fila,columna,usuario,tablero):
	...
	if not(es_valida_la_posicion(fila,columna,tablero)):
		return False
	for moverse_en_filas in range(1,-2,-1):
		for moverse_en_columnas in range(1,-2,-1):
			if not(moverse_en_filas==0 and moverse_en_columnas==0):
				fil=fila+moverse_en_filas
				col=columna+moverse_en_columnas
				if (esta_en_tablero(fil,col) and tablero[fil][col]==ficha_otro_usuario):
					ficha_a_convertir_en_mia.append([fil,col])
					fil=fil+moverse_en_filas  
					col=col+moverse_en_columnas
